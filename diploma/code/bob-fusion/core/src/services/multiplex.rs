...
where
    WebFuture: Future<Output = std::result::Result<Web, WebError>>,
{
    type Output = std::result::Result<HybridService<Web, Grpc>, WebError>;
...
fn is_grpc_request<Body>(req: &Request<Body>) -> bool {
    ...
    req.headers()
        ...
        .map(axum::headers::HeaderValue::as_bytes)
    ...
    GrpcBody::Error: Error + Send + Sync + 'static,
{
    ...
    fn poll_data(
        ...
        match self.project() {
            HybridBodyProj::Web(b) => b.poll_data(cx).map_err(std::convert::Into::into),
            HybridBodyProj::Grpc(b) => b.poll_data(cx).map_err(std::convert::Into::into),
        }
    }
    ...
    fn poll_trailers(
        ...
        match self.project() {
            HybridBodyProj::Web(b) => b.poll_trailers(cx).map_err(std::convert::Into::into),
            HybridBodyProj::Grpc(b) => b.poll_trailers(cx).map_err(std::convert::Into::into),
        }
    }

    fn is_end_stream(&self) -> bool {
        match self {
            Self::Web(b) => b.is_end_stream(),
...
where
    WebFuture: Future<Output = std::result::Result<Response<WebBody>, WebError>>,
    GrpcFuture: Future<Output = std::result::Result<Response<GrpcBody>, GrpcError>>,
